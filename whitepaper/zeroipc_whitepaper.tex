...
\begin{abstract}
We present ZeroIPC, a groundbreaking shared memory IPC library that transforms inter-process communication by implementing codata structures---lazy evaluation, futures, streams, and channels---alongside comprehensive synchronization primitives as lock-free constructs in shared memory. While traditional IPC systems treat shared memory as passive storage requiring serialization, ZeroIPC introduces an active computational substrate where functional programming abstractions and cross-process synchronization operate directly on memory-mapped regions. Our key innovation lies in bridging theoretical computer science with systems programming: we formalize codata semantics for distributed systems and provide efficient lock-free implementations using atomic compare-and-swap operations. Beyond codata, ZeroIPC provides a complete synchronization toolkit including mutexes, read-write locks, events, monitors with condition variables, and reactive signals with version tracking. The system defines a language-agnostic binary format enabling transparent interoperability between C++23, C99, and Python processes without marshaling overhead. Experimental evaluation demonstrates that ZeroIPC achieves millions of operations per second with comprehensive test coverage across 200+ test cases including cross-process validation and concurrency stress tests. By bringing concepts from category theory and functional reactive programming to the systems domain, ZeroIPC enables new patterns for building distributed applications where computation, communication, and synchronization are unified through shared memory. The library is available as open source, providing both a theoretical contribution to distributed computing and a practical tool for high-performance IPC.
\end{abstract}
...
\item \textbf{Complete synchronization toolkit}: Beyond codata, we provide cross-process synchronization primitives including mutexes, read-write locks, events with manual/auto-reset modes, monitors with condition variables, once-only initialization, and reactive signals with version tracking---all implemented as lock-free structures in shared memory.

\item \textbf{Comprehensive evaluation}: We demonstrate performance exceeding millions of operations per second, with extensive testing across 200+ test cases including ABA problem detection, memory boundary validation, cross-process validation, and multi-threaded stress tests.
\end{itemize}
...
Beyond codata structures, ZeroIPC provides a comprehensive set of cross-process synchronization primitives, all implemented using lock-free techniques on shared memory:

\textbf{Mutex}: Binary semaphore providing mutual exclusion with lock/unlock semantics. Implemented using atomic compare-and-swap with spin-wait and exponential backoff for contention.

\textbf{RWLock}: Reader-writer lock allowing multiple concurrent readers or a single exclusive writer. Writers have priority to prevent starvation.

\textbf{Event}: Windows-style event synchronization with manual-reset (all waiters released) and auto-reset (single waiter released) modes. Built on semaphores with atomic state tracking.

\textbf{Monitor}: Combines mutex with condition variable semantics, supporting wait/notify\_one/notify\_all operations. Implements predicate-based waiting to handle spurious wakeups correctly.

\textbf{Once}: Thread-safe one-shot initialization ensuring a callable executes exactly once across all processes, with automatic reset capability for testing.

\textbf{Signal}: Reactive value container with version tracking and change notification. Enables publish-subscribe patterns across processes with efficient wait-for-change semantics.

\begin{lstlisting}[caption={Synchronization Primitives Usage},label={lst:sync}]
// Monitor-based producer-consumer
Monitor mon(mem, "producer_consumer");
Array<int> buffer(mem, "buffer", 100);

// Producer
mon.lock();
mon.wait([&]() { return count < MAX; });
buffer[count++] = item;
mon.notify_one();
mon.unlock();
...
temperature.set(25.5);  // Updates with version

// Other process observes changes
while (true) {
    temp.wait_for_change(last_version);
    process_update(temp.get());
...
\begin{itemize}
\item Unit tests: 200+ test cases across C++ and Python implementations
\item Synchronization tests: Cross-process validation using multiprocessing
\item Error handling tests: Timeout behavior, double-unlock detection, missing resource handling
\item Integration tests: Cross-language interoperability verified (C++/Python)
...
Test suite optimization achieved 200x speedup through categorized execution (FAST/MEDIUM/SLOW/STRESS), enabling rapid development iteration while maintaining thorough validation.
...
ZeroIPC demonstrates that shared memory can transcend its traditional role as passive storage to become an active computational substrate. By implementing codata structures---futures, lazy evaluation, streams, and channels---alongside comprehensive synchronization primitives as lock-free shared memory constructs, we enable both functional programming patterns and robust process coordination across boundaries without serialization overhead.

Our contributions bridge theoretical computer science and systems programming. We formalized codata semantics for distributed systems, provided efficient lock-free implementations of both computational codata and synchronization primitives (mutex, read-write lock, event, monitor, once, and signal), and established a language-agnostic binary format for cross-language interoperability. Performance evaluation confirms that ZeroIPC achieves millions of operations per second while maintaining correctness under concurrent stress, validated through 200+ test cases including cross-process scenarios.

The impact extends beyond performance metrics. ZeroIPC enables new architectural patterns where computation, communication, and synchronization unify through shared memory. Scientific simulations can share lazy computations, reactive systems can process infinite streams, microservices can coordinate through CSP channels, and distributed processes can synchronize through monitors with condition variables---all with zero-copy efficiency.

As computing systems grow increasingly parallel and distributed, the need for efficient yet expressive IPC abstractions becomes critical. ZeroIPC provides both a theoretical framework and practical implementation for this challenge. By bringing functional programming's most powerful abstractions alongside proven synchronization patterns to the systems domain, we open new possibilities for building robust, performant distributed applications.

The open-source release of ZeroIPC invites the community to explore and extend these ideas. We believe codata and synchronization primitives in shared memory represent a fundamental advance in how processes communicate and collaborate, pointing toward a future where the boundaries between computation, communication, and coordination dissolve into a unified programming model.
